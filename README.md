...
| **IoT & Voice** | **Jarvis V2** *(En desarrollo)* | Asistente de voz local distribuido y multi-nodo: satélites Raspberry Pi 4 por habitación, Hub Central como orquestador y máquina GPU dedicada para inferencia LLM. Pipeline completo offline con auto-descubrimiento de herramientas vía Rasa NLU. Latencia E2E objetivo < 3 s. | `LangGraph`, `FastAPI`, `Ollama`, `Whisper.cpp`, `Piper TTS`, `Rasa NLU`, `Raspberry Pi 4`, `Porcupine` | [📄 Ver Detalle](./Jarvis_V2) |
...
| **DevOps & CI/CD** | **Autotest CI/CD Pipeline** | Pipeline CI/CD completo para monitorización de tags Git, compilación de DVDs/ISOs, análisis SonarQube, gestión de VMs y despliegue SSH automatizado, con Web UI de monitorización. | `Python`, `Flask`, `Alpine.js`, `Tailwind`, `SonarQube`, `vCenter` | [📄 Ver Detalle](./Autotest/cicd/) |
