...
#
# Ejecutar como usuario 'YOUR_USER' en la máquina de desarrollo (YOUR_PIPELINE_HOST_IP)
# Uso: ./setup_phase0.sh
#===============================================================================
...
check_user() {
    if [[ "$(whoami)" != "agent" ]]; then
        log_error "Este script debe ejecutarse como usuario 'YOUR_USER'"
...
setup_ssh_keys() {
    ...
    local target_vm="YOUR_TARGET_VM_IP"
...
show_summary() {
    ...
    echo "  3. Verificar SSH a la VM: ssh -i ~/.ssh/id_rsa YOUR_DEPLOY_USER@YOUR_TARGET_VM_IP"
