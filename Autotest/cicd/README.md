...
- Git
- yq (parser YAML)
- Acceso SSH a la VM destino (YOUR_TARGET_VM_IP)
- Acceso a Git (https://YOUR_GIT_SERVER/YOUR_ORG/YOUR_REPO)
- Acceso a SonarQube (https://YOUR_SONARQUBE_SERVER)
...
scp -r cicd/ YOUR_USER@YOUR_PIPELINE_HOST_IP:/home/YOUR_USER/
```
...
# - SONAR_TOKEN (obtener de YOUR_SONARQUBE_SERVER)
...
ssh-copy-id -i ~/.ssh/id_rsa.pub YOUR_DEPLOY_USER@YOUR_TARGET_VM_IP

# Probar conexión
ssh -i ~/.ssh/id_rsa YOUR_DEPLOY_USER@YOUR_TARGET_VM_IP "hostname"
```
...
git ls-remote https://YOUR_GIT_SERVER/YOUR_ORG/YOUR_REPO
```
...
ping -c 4 YOUR_VCENTER_IP

# Probar conexión HTTPS
curl -k https://YOUR_VCENTER_IP/
```
...
```bash
# Verificar token
curl -u "$SONAR_TOKEN:" "https://YOUR_SONARQUBE_SERVER/api/authentication/validate"
...
ssh -v -i ~/.ssh/id_rsa YOUR_DEPLOY_USER@YOUR_TARGET_VM_IP

# Ping a la VM
ping -c 4 YOUR_TARGET_VM_IP
...
ssh-keygen -R YOUR_TARGET_VM_IP
```
