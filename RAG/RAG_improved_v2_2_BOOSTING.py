...
# ==========================================================================
# v2.4: SISTEMA DE FILTRADO HÍBRIDO POR CARPETAS
# ==========================================================================
@dataclass
class SearchMode:
    """Modo de busqueda detectado"""
    mode: str
    target_folder: Optional[str]
    cleaned_query: str
    original_query: str
def detect_search_mode(query: str) -> SearchMode:
    """Detecta modo de busqueda segun keywords"""
    query_lower = query.lower()
    strict_patterns = [
        (r'solo\s+(\w+)', 'strict'),
        (r'unicamente\s+(\w+)', 'strict'),
        (r'only\s+(\w+)', 'strict'),
    ]
    for pattern, mode in strict_patterns:
        match = re.search(pattern, query_lower)
        if match:
            folder = match.group(1)
            cleaned = re.sub(pattern, '', query, flags=re.IGNORECASE).strip()
            logger.info(f"Modo ESTRICTO: {folder}")
            return SearchMode('strict', folder, cleaned, query)
    boosting_patterns = [
        (r'busca en\s+(\w+)', 'boosting'),
        (r'en\s+(\w+)\s+', 'boosting'),
    ]
    for pattern, mode in boosting_patterns:
        match = re.search(pattern, query_lower)
        if match:
            folder = match.group(1)
            cleaned = re.sub(pattern, '', query, flags=re.IGNORECASE).strip()
            logger.info(f"Modo BOOSTING: {folder}")
            return SearchMode('boosting', folder, cleaned, query)
    logger.info("Modo GLOBAL")
    return SearchMode('global', None, query, query)
def get_available_folders(docs_dir: str = DOCS_DIR) -> Dict[str, int]:
    """Lista carpetas en docs/"""
    import glob
    import os
    folders = {}
    ...
    for item in os.listdir(docs_dir):
        ...
        if os.path.isdir(item_path):
            ...
            for ext in ['*.pdf', '*.md', '*.txt']:
                count += len(glob.glob(os.path.join(item_path, '**', ext), recursive=True))
            if count > 0:
                # Normalizar nombre igual que en load_documents_with_metadata
                folder_normalized = item.split('_')[0].lower() if '_' in item else item.lower()
                folders[folder_normalized] = folders.get(folder_normalized, 0) + count
    return folders
def filter_results_by_folder(results: List[Tuple[Document, float]], folder: str, mode: str = 'strict') -> List[Tuple[Document, float]]:
    """Filtra resultados por carpeta"""
    if mode == 'strict':
        filtered = [(d, s) for d, s in results if d.metadata.get('folder', '').lower() == folder.lower()]
        logger.info(f"Filtrado: {len(filtered)}/{len(results)}")
        return filtered
    elif mode == 'boosting':
        BOOST = 2.0
        boosted = []
        for d, s in results:
            if d.metadata.get('folder', '').lower() == folder.lower():
...
class SimpleQueryExpander:
    ...
    def __init__(self):
        ...
        # v2.4: Expandido de 12 a 37 términos
        self.expansions = {
    ...
    def expand(self, query: str) -> Tuple[str, bool]:
        ...
        for term, synonyms in self.expansions.items():
            if term in query_lower:
                ...
                was_expanded = True
        # NUEVO v2.4: Búsqueda inversa en valores (bidireccional)
...
class FastReranker:
    ...
    def rerank(self, query: str, docs: List[Tuple[Document, float]], top_k: int = 5) -> List[Tuple[Document, float]]:
        ...
        for doc, original_score in docs[:20]:  # Solo top 20
            ...
            # NUEVO v2.4: Bonus para docs internos (usando metadata)
...
class ImprovedHybridRetriever:
    ...
    def __init__(self, vectorstore, documents: List[Document], base_alpha: float = 0.5):
        ...
        # NUEVO v2.4: Inicializar embedding cache
        self.embedding_cache = EmbeddingCache(max_size=1000)
        # NUEVO v2.4: Identificar documentos internos (.md) para boosting
        # FIX v2.4: Usar metadata en lugar de id() para evitar identity mismatch
    ...
    def retrieve(self, query: str, k: int = 20) -> List[Tuple[Document, float]]:
        ...
        try:
            # NUEVO v2.4: Intentar usar cache de embeddings
        ...
        # Combinar scores
        # NUEVO v2.4: Aplicar BOOSTING a docs internos
        # v2.4: Aumentado a 75% para priorizar más agresivamente
...
def load_documents_with_metadata(docs_dir: str) -> List[Document]:
    ...
    for file_path in files:
        try:
            ...
            if ext == '.pdf':
                ...
                continue
            # v2.4: Metadata de carpeta
            rel_path = os.path.relpath(file_path, docs_dir)
            folder_raw = os.path.dirname(rel_path).split(os.sep)[0] if os.path.dirname(rel_path) else 'root'

            # v2.4 FIX: Normalizar nombre de carpeta para matching flexible
            # "Conocimiento_entorno" -> "conocimiento", "manuales" -> "manuales"
            folder = folder_raw.split('_')[0].lower() if '_' in folder_raw else folder_raw.lower()

            for doc in docs:
                doc.metadata['file_type'] = ext
                doc.metadata['file_name'] = os.path.basename(file_path)
                doc.metadata['folder'] = folder  # Normalizado para matching
                doc.metadata['folder_full'] = folder_raw  # Original completo
    ...
    folders = {}
    for doc in all_docs:
        f = doc.metadata.get('folder', 'unknown')
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("SISTEMA RAG v2.4.1 - VMware ESXi (FOLDER FILTERING FIXED)")
    logger.info("v2.4.1: Filtrado por carpetas arreglado + Nombres normalizados")
    logger.info("=" * 60)
    # Obtener vectorstore
    ...
    logger.info("✓ Sistema listo (v2.4 - boosting arreglado + cache)")
    print("\n" + "=" * 70)
    print("EXPERTO EN VMWARE ESXi (RAG v2.4 - BOOSTING FIXED)")
    print("=" * 70)
    print("Mejoras v2.4:")
    ...
    while True:
        try:
            ...
            if query.lower() == 'stats':
                try:
                    ...
                    with open(manifest_path, 'r') as f:
                        manifest = json.load(f)
                    # NUEVO v2.4: Estadísticas de cache
                    ...
                    print(f"  - Versión: v2.4 (boosting fixed + cache)")
                    ...
                    # Carpetas
                    folders = get_available_folders(DOCS_DIR)
                    print("")
                    print("[F] Carpetas:")
                ...
                continue
            # v2.4: Comando carpetas
            if query.lower() in ['carpetas', 'folders']:
                try:
                    folders = get_available_folders(DOCS_DIR)
                    print("")
                    ...
                    print("")
                    print("[i] Uso:")
                    print("  busca en [carpeta] = prioriza")
                    print("  SOLO [carpeta] = exclusivo")
                except Exception as e:
                    print(f"Error: {e}")
                continue
            query_count += 1
            ...
            if search_mode.mode == "strict":
                print(f"[!] EXCLUSIVO en: {search_mode.target_folder}")
            elif search_mode.mode == "boosting":
                print(f"[+] Priorizando: {search_mode.target_folder}")
            working_query = search_mode.cleaned_query
            # Query Expansion (sin LLM, instantáneo)
            query_expanded, was_expanded = query_expander.expand(working_query)
            if was_expanded:
            ...
            if not results:
                ...
                continue
            # v2.4: Aplicar filtrado
            if search_mode.mode in ["strict", "boosting"]:
                results = filter_results_by_folder(results, search_mode.target_folder, search_mode.mode)
                if not results and search_mode.mode == "strict":
                    print(f"[!] Sin resultados en {search_mode.target_folder}")
                    continue
